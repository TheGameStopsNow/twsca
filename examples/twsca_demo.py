...
def plot_signals_and_analysis(t, signal1, signal2, twsca_result, title=None, output_file=None):
    """
    ...
    axes[0, 1].plot(aligned_t, twsca_result["aligned_series1"], label="Signal 1 (aligned)")
    axes[0, 1].plot(aligned_t, twsca_result["aligned_series2"], label="Signal 2 (aligned)")
    axes[0, 1].set_title(
    ...
    if "warping_path" in twsca_result:
        ...
        axes[1, 0].plot([0, path_max], [0, path_max], "r--", alpha=0.5, label="No warping")
        axes[1, 0].legend()
...
def main():
    parser = argparse.ArgumentParser(description="Demonstrate TWSCA with synthetic data")
    parser.add_argument(
        "--points", type=int, default=100, help="Number of points in the time series"
    )
    parser.add_argument(
        "--phase", type=float, default=1.0, help="Phase shift between the two signals (in radians)"
    )
    parser.add_argument(
        "--warp", action="store_true", help="Apply non-linear time warping to the second signal"
    )
